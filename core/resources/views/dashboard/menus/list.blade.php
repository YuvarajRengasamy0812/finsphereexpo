...
@section('content')
    ...
    @foreach ($ParentMenus as $ParentMenu)
        <div id="mg-{{ $ParentMenu->id }}" class="modal fade" data-backdrop="true">
            <div class="modal-dialog" id="animate">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('backend.confirmation') }}</h5>
                                    ...
                                    @foreach ($ParentMenus as $ParentMenu)
                                        ...
                                        <li style="margin-bottom: 5px">
                                            <a class="btn white {{ $ParentMenuId == $ParentMenu->id ? 'active' : '' }} w-100"
                                                ...
                                                {!! $ParentMenu->id == $edt_id ? " style='font-weight: bold;color:#976737'" : '' !!}>
                                                {!! $title !!}
                                            </a>
                                    ...
                                    @foreach (Helper::languagesList() as $ActiveLanguage)
                                        @if ($ActiveLanguage->box_status)
                                            <div class="form-group row">
                                                <div class="col-sm-12">
                                                    {!! Form::text('title_' . @$ActiveLanguage->code, $EditedMenu->{'title_' . @$ActiveLanguage->code}, [
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12">
                                            <span class="input-group-btn">
                                                <button class="btn btn-fw primary"
                                        ...
                                        <div class="col-sm-12">
                                            <span class="input-group-btn">
                                                <button class="btn btn-sm primary"
                                                    type="submit">{!! __('backend.add') !!}</button>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($ParentMenuId > 2)
                                <div class="pull-right">
                                    ...
                                    <button class="btn btn-sm warning" data-toggle="modal"
                                        data-target="#mg-{{ $ParentMenuId }}" ui-toggle-class="bounce"
                                        title="{{ __('backend.delete') }}" ui-target="#animate"><i
                                        ...
                                        @foreach ($Menus as $Menu)
                                            ...
                                            <tr>
                                                <td class="dker"><label class="ui-check m-a-0">
                                                        <input type="checkbox" name="ids[]"
                                                            value="{{ $Menu->id }}"><i class="dark-white"></i>
                                                        {!! Form::hidden('row_ids[]', $Menu->id, ['class' => 'form-control row_no']) !!}
                                                    </label>
                                                </td>
                                                <td class="h5">
                                                    {!! Form::text('row_no_' . $Menu->id, $Menu->row_no, [
                                                        'class' => 'form-control row_no light',
                                                        'id' => 'row_no',
                                                        'autocomplete' => 'off',
                                                    ]) !!}
                                                    @if ($Menu->icon)
                                                ...
                                                <td class="text-center">
                                                    <i
                                                        class="fa {{ $Menu->status == 1 ? 'fa-check text-success' : 'fa-times text-danger' }} inline"></i>
                                                </td>
                                                <td class="text-center">
                                                    <a class="btn btn-sm success"
                                            ...
                                            @foreach ($Menu->subMenus as $subMenu)
                                                ...
                                                <tr>
                                                    <td class="dker"><label class="ui-check m-a-0">
                                                            <input type="checkbox" name="ids[]"
                                                                value="{{ $subMenu->id }}"><i class="dark-white"></i>
                                                            {!! Form::hidden('row_ids[]', $subMenu->id, ['class' => 'form-control row_no']) !!}
                                                        </label>
                                                    </td>
                                                    <td class="h5">
                                                        <img src="{{ asset('assets/dashboard/images/treepart_' . @Helper::currentLanguage()->direction . '.png') }}"
                                                            class="submenu_tree">
                                                        {!! Form::text('row_no_' . $subMenu->id, $subMenu->row_no, [
                                                            'class' => 'form-control row_no light',
                                                            'id' => 'row_no',
                                                            'autocomplete' => 'off',
                                                        ]) !!}
                                                    ...
                                                    <td class="text-center">
                                                        <i
                                                            class="fa {{ $subMenu->status == 1 ? 'fa-check text-success' : 'fa-times text-danger' }} inline"></i>
                                                    </td>
                                                    <td class="text-center">
                                                        <a class="btn btn-sm success"
                                                    ...
                                                    <tr>
                                                        <td class="dker"><label class="ui-check m-a-0">
                                                                <input type="checkbox" name="ids[]"
                                                                    value="{{ $subMenu2->id }}"><i
                                                                    class="dark-white"></i>
                                                                {!! Form::hidden('row_ids[]', $subMenu2->id, ['class' => 'form-control row_no']) !!}
                                                            </label>
                                                        </td>
                                                        <td class="h5">
                                                            <img src="{{ asset('assets/dashboard/images/treepart_sub_' . @Helper::currentLanguage()->direction . '.png') }}"
                                                                class="submenu_tree">
                                                            {!! Form::text('row_no_' . $subMenu2->id, $subMenu2->row_no, [
                                                                'class' => 'form-control row_no light',
                                                                'id' => 'row_no',
                                                                'autocomplete' => 'off',
                                                            ]) !!}
                                                        ...
                                                        <td class="text-center">
                                                            <i
                                                                class="fa {{ $subMenu2->status == 1 ? 'fa-check text-success' : 'fa-times text-danger' }} inline"></i>
                                                        </td>
                                                        <td class="text-center">
                                                            <a class="btn btn-sm success"
